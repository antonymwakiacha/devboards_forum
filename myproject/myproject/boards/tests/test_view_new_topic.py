...
class  NewTopicTests(TestCase):
    ...
    def test_contains_form(self):
    	...
    	self.assertIsInstance(form,NewTopicForm)#grabs the form instance in the context data and checking if its a NewTopicForm
	...
	def test_new_topic_invalid_post_data(self):
		...
		form=response.contect.get('form')
		self.assertEquals(response.status_code,200)
		self.assertTrue(form.errors)#makes sure the form is d=showing errors when the data is invalid
...
#test that tries to make a request to the new topic view without being authenticated.THe expected result is for the request be redirected to the login view

class LoginRequiredNewTopicTests(TestCase):
	def setUp(self):
		Board.objects.creare(name='Django',description='Django board.')
